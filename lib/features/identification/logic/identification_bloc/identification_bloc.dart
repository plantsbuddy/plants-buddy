...
class IdentificationBloc extends Bloc<IdentificationEvent, IdentificationState> {
  ...
  final SaveIdentificationForHistory _saveIdentificationForHistory;

  IdentificationBloc(this._captureImageFromCamera, this._downloadImageFromUrl, this._pickImageFromGallery,
      this._performIdentification, this._saveIdentificationForHistory,
  ...
  FutureOr<void> onIdentificationPerformIdentificationPressed(
    ...
    await _saveIdentificationForHistory(
        identificationType: state.identificationType, results: predictions ?? [], imagePath: state.image!);
  }
}
